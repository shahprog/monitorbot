...
class Monitor(commands.Cog):
    ...
    async def all(self, ctx, group: str = "bot", channel: discord.TextChannel = None):
        ...
        if channel is not None:
            ...
            if not bot.permissions_in(channel).send_messages:
                return await ctx.send(
                    "Sorry! I cant send message on that channel. Use different channel or give me permission to send message on that channel."
                )
            else:
                channel_id = channel.id
        else:
        ...
        if group == "bot":
            ...
            monitors = [
                member.id for member in ctx.guild.members if member.bot == False
            ]
        ...
        question = await ctx.send(
            f"Are you sure you want to monitor `{len(monitors)}` {group}? Write yes or no in 60 seconds..."
        )
    ...
    async def _ls(self, ctx):
        ...
        data = await self.bot.db.fetch(
            "SELECT * FROM monitor WHERE mentor = $1 and guild=$2",
            ctx.author.id,
            ctx.guild.id,
        )
        ...
        pages = menus.MenuPages(
            source=MySource(targets, f"Monitor of {str(ctx.author)}"),
    ...
    async def dl_(self, ctx, monitors: commands.Greedy[discord.Member]):
        ...
        for monitor in monitors:
            ...
            if data:
                await self.bot.db.execute(
                    "DELETE FROM monitor WHERE target = $1 and mentor = $2 and guild = $3",
                    monitor,
                    ctx.author.id,
                    ctx.guild.id,
                )
    ...
    async def clear(self, ctx):
        ...
        await self.bot.db.execute(
            "DELETE FROM monitor WHERE mentor = $1 and guild = $2",
            ctx.author.id,
            ctx.guild.id,
        )
